...
<body class="bg-gray-50 flex min-h-screen" x-data="{ sidebarOpen: false, sidebarCollapsed: false }">

    <!-- Overlay untuk mobile (diperbarui: efek blur + transparan) -->
    <div 
    ...
    <!-- Sidebar -->
    <aside 
        class="fixed top-0 left-0 z-40 h-full bg-white border-r border-gray-200 transition-all duration-300 transform
               lg:translate-x-0"
        :class="{
        ...
        x-on:resize.window.debounce="if(window.innerWidth >= 1024) sidebarOpen = false"
    >
        <div class="flex flex-col h-full">

            <!-- Header Sidebar -->
            <div class="flex items-center justify-between h-16 px-5 border-b border-gray-200">
                <a href="{{ url('/') }}" class="flex items-center gap-2 overflow-hidden transition-all"
                   :class="sidebarCollapsed ? 'opacity-0 w-0' : 'opacity-100'">
                    ...
                    <span class="text-base font-semibold text-gray-900">EduDashboard</span>
                </a>

                <!-- Tombol collapse (desktop) -->
                <button class="hidden lg:flex p-1.5 rounded hover:bg-gray-100" 
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto text-sm">
                @php
                    $menuItems = [
                        ...
                        ['route' => 'orangtua', 'url' => null, 'label' => 'Dashboard Orang Tua dan siswa', 'icon' => '<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>'],
                    ];
                @endphp

                @foreach($menuItems as $item)
                    <a href="{{ $item['url'] ? url($item['url']) : route($item['route']) }}"
                       class="flex items-center rounded-lg font-medium transition
                       {{ Route::is($item['route']) || ($item['route'] === 'home' && request()->is('/')) ? 'bg-blue-600 text-white hover:bg-blue-700' : 'text-gray-700 hover:bg-gray-100' }}
                       px-4 py-3 gap-3">
