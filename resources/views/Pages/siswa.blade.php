...
@section('content')
  ...
  <div class="grid gap-6 lg:grid-cols-3">
    ...
    <div class="space-y-6 lg:col-span-2">
      ...
      <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
        ...
        <div class="mb-4">
          <button class="rounded-md bg-purple-600 px-4 py-2 font-semibold text-white hover:bg-purple-800">UAS</button>
          <button class="rounded-md bg-purple-600 px-4 py-2 font-semibold text-white hover:bg-purple-800">UTS</button>
        </div>
          <div class="space-y-4">
            @foreach ($siswa->uas as $uas)
              ...
              <div class="h-2 w-full rounded-full bg-purple-50 dark:bg-purple-950">
                <div class="h-2 rounded-full bg-purple-600" style="width: {{ $uas->nilai ?? 0 }}%"></div>
              </div>          
              @endforeach
          </div>
      </div>
    ...
    <div
      ...
      <div class="grid w-full grid-cols-2 gap-4">
        <div class="flex h-full flex-col rounded-lg bg-gray-50 p-4 pt-6 shadow-sm dark:bg-gray-800">
          ...
          <p class="mt-1 text-base leading-relaxed text-gray-700 dark:text-gray-300">sangat berkembang</p>
        </div>

        <div class="flex h-full flex-col rounded-lg bg-gray-50 p-4 pt-6 shadow-sm dark:bg-gray-800">
          <b class="text-gray-800 dark:text-white">Catatan UAS Siswa</b>
          <p class="mt-1 text-base leading-relaxed text-gray-700 dark:text-gray-300">Performa Siswa Menurun</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    new Chart(ctx, {
      type: 'radar',
      data: {
        labels: ['Tai', 'Iclik', 'Yapping', 'berak', 'titit', 'pele'],
        datasets: [{
          label: 'Potensi Siswa (Nilai UAS)',
          data: [85, 78, 90, 70, 88, 95],
